...
function genOpponents(f){
  ...
  const isTitle=(!isDefense && f.org>=1 && ((f.streak||0)>=3 || divRank(f)<=2));
  ...
  return `<div class="scr">
   ...
   <div class="glass mwash" style="position:relative;display:flex;justify-content:space-between;align-items:flex-start;gap:12px;background:var(--panel2);border:1px solid var(--line);padding:14px;margin-bottom:16px">
     <div style="position:relative;z-index:2">
       ...
       <div class="mono small" style="margin-top:2px">${champ?`<b class="gold">CHAMP. ${orgDisplayName(f).toUpperCase()}</b>`:((f.W+f.L+(f.D||0))===0||divRank(f)>15?'RANG: NR':`RANG: #${divRank(f)}`)}</div>
       ${declineHtml}
     </div>
...
function scr_select(){ const f=G.f;
  ...
  G.opps.forEach((e,i)=>{ const o=e.o;
    ...
    const rTag=o.champion?'CHAMPION':((fightsTot===0||rnk>15)?'NR':(rnk===1?'CHALLENGER #1':`RANG #${rnk}`));
    ...
    h+=`<div style="margin-bottom:32px;padding-bottom:24px;border-bottom:1px solid var(--line)">
      ...
      <button class="btn ${isRival?'primary':''}" style="margin-top:4px;font-size:15px;letter-spacing:.05em" onclick="CL.opp(${i})">${isRival?'RÉGLER SES COMPTES':'ACCEPTER LE COMBAT'}</button>
    </div>`;
  });
...
function scr_result(){ const p=G.pending,f=G.f,st=p.res.stats;
  ...
  if(p.method && p.method.startsWith('Déc') && p.res.judges){
    ...
    judgesHtml=`<div class="card gold-b" style="text-align:center">
      ...
      <div class="disp" style="font-size:22px;letter-spacing:1px;display:flex;justify-content:center;gap:16px">
        <span class="${J.j1[0]>J.j1[1]?'gold':'muted'}">${J.j1[0]}-${J.j1[1]}</span>
        <span class="${J.j2[0]>J.j2[1]?'gold':'muted'}">${J.j2[0]}-${J.j2[1]}</span>
        <span class="${J.j3[0]>J.j3[1]?'gold':'muted'}">${J.j3[0]}-${J.j3[1]}</span>
      </div>
      <div class="hr"></div>
...
function scr_profile(){ const f=G.f; const g=groupAvg(f);
  ...
  return `<div class="scr"><div class="bar"><span class="eyebrow">Fiche complète</span><span class="eyebrow x" onclick="CL.go('hub')">✕</span></div>
   <div class="card"><div class="fh"><div class="fh-l"><div class="nm">${esc(f.name)}</div>${f.nick?`<div class="nick">« ${f.nick} »</div>`:''}<div class="sub">${f.styleLabel} · ${f.divName} · ${f.age} ans · ${f.phys.height}cm/${f.phys.reach}cm</div></div><div class="fl">${f.flag}</div></div>
     ...
     ${(f.amaTitles&&f.amaTitles.length)?`<div class="tagrow" style="display:flex;flex-wrap:wrap;gap:6px;margin-top:8px">${f.amaTitles.map(id=>{const cfg=AMA_CHAMPIONSHIPS.find(c=>c.id===id); return cfg?`<span class="tag gold">Champion ${cfg.label}</span>`:'';}).join('')}</div>`:''}
...
function scr_toptier(){
  return `<div class="scr center intro"><div class="eyebrow gold">Le Sommet du Monde</div>
   ...
   <div class="card" style="border-color:var(--blood);text-align:left">
     <h3 class="disp" style="margin:0;color:var(--blood)">PACIFIC CHAMPIONSHIP (Gloire)</h3>
     ...
     <button class="btn primary" onclick="CL.signTopTier(5)">Signer chez Pacific Championship</button>
   </div>
   <div class="card" style="border-color:var(--sage);text-align:left;margin-top:15px">
     <h3 class="disp" style="margin:0;color:var(--sage)">ULTIMATE RIM (Argent & Santé)</h3>
     ...
     <button class="btn" style="background:var(--sage);color:#fff;border:none" onclick="CL.signTopTier(6)">Signer chez Ultimate Rim</button>
   </div></div>`; }
/* ==== [FIN ANCRE] ==== */
...
const CL={
  ...
  acceptPro(){ turnPro(); G.f.org=1; if(ORG_FLAVORS[1]) G.f.orgFlavor=pick(ORG_FLAVORS[1]); G.roster=makeOrgRoster(G.f,'PRO_TRANSITION'); if(G.pending)G.pending.proOffer=null; G.screen='hub'; save(); render(); },
...
function scr_arena(){ const A=ARENA||{};
  return `<div class="fade"><div class="eyebrow center">${esc(A.nmeName||'')} ${A.meFlag||''} vs ${A.opFlag||''} ${esc(A.nopName||'')}</div>
   <div class="card glass raise" style="padding:10px;border-color:var(--gold-d);background:var(--panel2)">
     ...
     <div class="arena-hud">
       <div style="display:flex;flex-direction:column">
         ...
         <div style="display:flex;gap:4px;margin-top:3px;align-items:center">
           <span style="font-size:9px;color:var(--muted)">Tête</span><div id="dm-h" style="width:10px;height:10px;background:var(--sage)"></div>
           <span style="font-size:9px;color:var(--muted);margin-left:4px">Corps</span><div id="dm-b" style="width:10px;height:10px;background:var(--sage)"></div>
           <span style="font-size:9px;color:var(--muted);margin-left:4px">Jambes</span><div id="dm-l" style="width:10px;height:10px;background:var(--sage)"></div>
         </div>
       </div>
       <div style="display:flex;flex-direction:column;align-items:flex-end">
         <span class="ah-name sage">${esc(A.nopName||'Adv.')}</span>
         <div style="display:flex;gap:4px;margin-top:3px;align-items:center">
           <span style="font-size:9px;color:var(--muted)">Tête</span><div id="do-h" style="width:10px;height:10px;background:var(--sage)"></div>
           <span style="font-size:9px;color:var(--muted);margin-left:4px">Corps</span><div id="do-b" style="width:10px;height:10px;background:var(--sage)"></div>
           <span style="font-size:9px;color:var(--muted);margin-left:4px">Jambes</span><div id="do-l" style="width:10px;height:10px;background:var(--sage)"></div>
         </div>
       </div>
     </div>
