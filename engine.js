...
function simulateFight(A,B,rounds=3,plan=null){ const a=eff(A),b=eff(B);
  ...
  for(let r=1;r<=rounds && !finish;r++){
    ...
    let sA=10,sB=10;
    if(rDiff>28||kdDiff>=3){ sA=10;sB=7; }
    else if(rDiff>15||kdDiff>=2){ sA=10;sB=8; }
    else if(rDiff>3||kdDiff===1){ sA=10;sB=9; }
    else if(rDiff<-28||kdDiff<=-3){ sA=7;sB=10; }
    else if(rDiff<-15||kdDiff<=-2){ sA=8;sB=10; }
