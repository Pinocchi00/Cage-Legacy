...
/* ==== [ANCRE: CORRECTIF_COMBAT_ORPHELIN] — teste le correctif du même nom
   (ui-08, CL.cont()) : choosePlan() résout et sauvegarde le combat AVANT que
   le joueur ne voie l'arène. Un onglet tué pendant l'animation (le scénario
   qu'un rechargement simule ici) ne doit ni perdre le mois Faith du combat
   déjà joué, ni forcer un retour muet au hub — le joueur doit retrouver
   l'écran de résultat, pouvoir cliquer Continuer, et voir le calendrier
   avancer normalement. ==== */
test('CORRECTIF_COMBAT_ORPHELIN — un rechargement pendant l’arène reprend sur le résultat, pas le hub', () => {
  const win = newGameWindow({ runMain: true });
  win.CL.startFaith();
  const d = win.G.faithDraft;
  Object.assign(d, {
    first: 'Orphelin', div: win.eval("DIVISIONS.H[0].id"), origin: 'traditional',
  ...
  // Joue jusqu'à ce que choosePlan() ait résolu un combat. En Faith, render()
  // redirige immédiatement 'arena' vers 'faith_fight_pending' (ANCRE
  // V3_FAITH_FIGHT_PENDING — Faith ne montre jamais l'arène round par
  // round) : G.screen n'est donc JAMAIS observably 'arena' ici, mais
  // resolveFight() a déjà tourné et déjà sauvegardé au moment où ce nouvel
  // écran s'affiche — exactement le scénario visé (un onglet tué juste
  // après avoir tapé "Lancer le combat", avant même l'animation).
  const monthBefore = win.G.faith.month;
  clickThrough(win, { maxSteps: 400, stopWhen: w => w.G.screen === 'faith_fight_pending' });
  assert.equal(win.G.screen, 'faith_fight_pending', 'le combat doit avoir été lancé (choosePlan atteint)');
  assert.ok(win.G.pending, 'resolveFight() doit avoir posé G.pending avant même que l’écran de verdict ne s’affiche');
  assert.equal(win.G.pending._consumed, undefined, 'G.pending ne doit pas encore être marqué consommé');

  // Simule le rechargement : nouvel appel à load() (comme le ferait cont()
  // sur un onglet fraîchement rouvert), qui relit exactement ce que
  // choosePlan() a déjà persisté via son propre save().
  win.CL.cont();
  assert.equal(win.G.screen, 'result', 'cont() doit reprendre sur l’écran de résultat, jamais le hub, tant que G.pending n’est pas consommé');

  // Le joueur clique enfin "Continuer" : le mois Faith doit être consommé
  // une seule fois, normalement — rien n’a été perdu ni dupliqué.
  win.CL.afterResult();
  assert.equal(win.G.pending._consumed, true, 'afterResult() doit marquer G.pending consommé');
  assert.ok(win.G.faith.month > monthBefore || win.G.screen === 'faith_year_end',
    'le mois Faith du combat doit être consommé au premier afterResult(), pas perdu par le rechargement');
});

/* ==== [ANCRE: CORRECTIF_DOUBLE_AFTERRESULT] — teste le verrou lui-même :
   un double-tap sur "Continuer" ne doit consommer qu'UN seul mois Faith. ==== */
test('CORRECTIF_DOUBLE_AFTERRESULT — un double-tap sur Continuer ne consomme le mois Faith qu’une fois', () => {
  const win = newGameWindow({ runMain: true });
  win.CL.startFaith();
  const d = win.G.faithDraft;
  Object.assign(d, {
    first: 'DoubleTap', div: win.eval("DIVISIONS.H[0].id"), origin: 'traditional',
    style: win.eval("Object.keys(STYLES)[0]"), lifestyle: 'pro', circle: 'family',
    agent: win.eval("Object.keys(FAITH_AGENTS)[0]"), personality: 'taiseux', stable: 'regional',
  });
  win.CL.finalizeFaithDraft();
  ...
  win.CL.afterResult(); // double-tap : doit être un no-op complet
  assert.equal(win.G.faith.month, monthAfterFirst, 'un second afterResult() ne doit pas avancer le calendrier une deuxième fois');
  assert.equal(win.G.screen, screenAfterFirst, 'un second afterResult() ne doit pas non plus changer l’écran');
  assert.ok(monthAfterFirst > monthBefore || screenAfterFirst === 'faith_year_end', 'le premier afterResult() doit, lui, avoir bien avancé le calendrier');
});
