...
test('MGMT T1 migration — v4 → v5 sans perte : les combats d\u2019une carte en cours deviennent des préliminaires', () => {
  ...
  /* Version courante en symbole : le tampon suit toutes les migrations. */
  ...
  assert.equal(win.eval(`G.mgmt&&G.mgmt.v`), win.eval(`MGMT_SAVE_VERSION`), 'une v4 se charge en version courante');
...
test('MGMT T1 migration — v2 et v3 migrent en chaîne 2 → 3 → 4 → 5', () => {
  ...
  assert.equal(mig3.v, win.eval(`MGMT_SAVE_VERSION`), 'migration séquentielle 3 → 4 → 5 → 6 → 7');
  ...
  assert.equal(mig2.v, win.eval(`MGMT_SAVE_VERSION`), 'migration séquentielle 2 → 3 → 4 → 5 → 6 → 7');
